<template>
	<div
		class="group relative flex h-[90svh] w-full flex-col items-center justify-between gap-4 pt-4"
	>
		<slot />
		<div class="buttons">
			<div v-for="(_, index) in Array(numOfPages)" :key="index">
				<input
					...
					:class="{
						...
						'scale-90 border border-blue-950 bg-transparent':
							index === activePage - 1,
					}"
...
const { numOfPages = 1, activePage = 1 } = defineProps([
	"numOfPages",
	"activePage",
]);
